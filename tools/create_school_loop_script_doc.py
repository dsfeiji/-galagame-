...
OUTPUT = r"C:\Users\33574\Documents\剧本杀\docs\学校循环剧本第一版.docx"
...
def build_doc():
    ...
    doc.add_heading("四 循环和保留规则", level=1)
    ...
    doc.add_heading("五 小游戏设计", level=1)
    ...
    doc.add_heading("六 七条学生路线", level=1)
    for route in routes:
    ...
    doc.add_heading("七 老师最终线", level=1)
    ...
    doc.add_heading("八 推荐全收集路线", level=1)
    ...
    doc.add_heading("九 阶段详细事件", level=1)
    stage_details = [
        ("阶段 1 校门口和走廊", [
    ...
    doc.add_heading("十 关键对话样稿", level=1)
    ...
    doc.add_heading("十一 JSON 落地建议", level=1)
    ...
    doc.add_heading("十二 第一版制作优先级", level=1)
